...
- 정기 리밸런싱 기준일은 7일입니다. 기준일이 없는 최초 리밸런싱이나 수동 리밸런싱에서는
  최근 14일의 원문·요약을 함께 읽어 메르의 논지 흐름을 유지하십시오.
- `rebalance` 실행에서는 과거 근거 보강으로 유지 중인 종목과 목표비중 8% 이상인 종목을 반드시
  재검토하십시오. 특히 `legacy_backfill_source`, `historical_backfill_2026-06-15`,
  `legacy_evidence_backfill_2026-06-15`가 붙은 종목은 근거가 복구되었다는 사실만으로 현재 비중을
  정당화하지 마십시오.
- `risk` 역할 종목이 8% 이상이거나, `satellite` 역할 종목이 6% 이상이거나, 개별 종목이 10%
  이상이면 반드시 유지/축소/매도 중 하나를 판단하고 이유를 기록하십시오. 유지한다면 왜 현금성
  20% 기준보다 해당 비중을 우선하는지 설명하십시오.
- 과거 AI가 제안한 10% 비중은 현재 정책의 기본값이 아닙니다. 최신 근거와 포트폴리오 위험을
  다시 평가해 필요하면 5~8% 수준으로 낮추고 줄인 비중은 현금성 자산으로 돌리십시오.
- 7일이 지나지 않았더라도 큰 이벤트가 있으면 `regular` 분석에서도 즉시 방어 조정 후보를
